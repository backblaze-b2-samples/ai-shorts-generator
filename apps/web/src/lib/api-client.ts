...
// --- AI Shorts jobs + clips ---

/** Start a shorts job: upload the source video and enqueue processing. */
export function startJob(
  file: File,
  opts: { clipCount: number; aspect: string },
  onProgress?: (percent: number) => void,
  ...
  return new Promise((resolve, reject) => {
    ...
    const formData = new FormData();
    formData.append("file", file);
    formData.append("clip_count", String(opts.clipCount));
    ...
    xhr.addEventListener("load", () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        try {
          const body = JSON.parse(xhr.responseText);
          reject(new ApiError(body.detail || `Job failed: ${xhr.status}`, xhr.status));
        } catch {
          reject(new ApiError(`Job failed: ${xhr.status}`, xhr.status));
        }
      }
    });
    xhr.addEventListener("error", () =>
...
export async function getClips() {
  return apiFetch<ClipItem[]>("/clips");
}

export async function getClipsStats() {
  return apiFetch<ClipsStats>("/clips/stats");
}

export async function getClipPreviewUrl(key: string) {
  return apiFetch<{ url: string }>(`/clips/${key}/preview`);
}

export async function getClipDownloadUrl(key: string) {
  return apiFetch<{ url: string }>(`/clips/${key}/download`);
}
